...
int main()
{
		...
		//////////Prueba enviar
		ETHER_HDR *ehdr = new ETHER_HDR();
		memcpy(ehdr->source, "00:0a:95:9d:68:16", 6); //Source Mac address
		memcpy(ehdr->dest, "00:0a:95:9d:68:17", 6); //Destination MAC address
		ehdr->type = 0x0800;// htons(0x0800); //IP Frames

		const char *dump = "CACACACACAA";
		...
		ip_hdr *iphdr = (PIPV4_HDR)(packet + sizeof(ETHER_HDR));

		iphdr->ip_version = 4;
		iphdr->ip_header_len = 5; //In double words thats 4 bytes
		iphdr->ip_tos = 0;
		iphdr->ip_total_length = 23;// htons(sizeof(IPV4_HDR) + sizeof(TCP_HDR) + strlen(dump));
		iphdr->ip_id = 2;// htons(2);
		...
		iphdr->ip_srcaddr = 123;// inet_addr("1.2.3.4");   //srcip.s_addr;
		iphdr->ip_destaddr = 567;// inet_addr("1.2.3.5");
		iphdr->ip_checksum = 0;
		iphdr->ip_checksum = 1234;//in_checksum((unsigned short*)iphdr, sizeof(IPV4_HDR));

		//InetPton(AF_INET, "1.2.3.4", &iphdr->ip_srcaddr);
		//InetPton(AF_INET, "5.6.7.8", &iphdr->ip_destaddr);

		// *******************  TCP Header *****************
		tcp_header *tcphdr = (PTCP_HDR)(packet + sizeof(ETHER_HDR) + sizeof(IPV4_HDR));

		tcphdr->source_port = 1234;// htons(1234);
		tcphdr->dest_port = 80;// htons(80);
		tcphdr->sequence = 0;
		...
		tcphdr->ecn = 0;
		tcphdr->cwr = 0;
		tcphdr->window = 64240;// htons(64240);
		tcphdr->checksum = 0;
		tcphdr->urgent_pointer = 0;

		
		char *data = (char*)(packet + sizeof(ETHER_HDR) + sizeof(IPV4_HDR) + sizeof(TCP_HDR));
		strcpy(data, dump);

		pcap_sendpacket(adhandle, packet, sizeof(ETHER_HDR) + sizeof(IPV4_HDR) + sizeof(TCP_HDR) + strlen(dump));
